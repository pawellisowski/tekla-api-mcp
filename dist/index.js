    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Tool execution failed for "${name}": ${errorMessage}`);
            }
        });
    }
...
`;
            ...
            try {
                if (detailed.inheritance &&
                    ...
                    const validHierarchy = detailed.inheritance.hierarchy.filter((item) => item && typeof item === 'string' && item.trim());
...
`;
            ...
            try {
                if (detailed.constructors && Array.isArray(detailed.constructors) && detailed.constructors.length > 0) {
                    const validConstructors = detailed.constructors.filter((ctor) => ctor && typeof ctor === 'object');
                    if (validConstructors.length > 0) {
                        output += `## Constructors

| Name | Description |
|------|-------------|
${validConstructors.map((ctor) => `| **${ctor.name || 'Unknown'}** | ${ctor.description || 'No description available'} |`).join('\n')}
...
`;
            ...
            try {
                if (detailed.properties && Array.isArray(detailed.properties) && detailed.properties.length > 0) {
                    const validProperties = detailed.properties.filter((prop) => prop && typeof prop === 'object');
                    if (validProperties.length > 0) {
                        output += `## Properties

| Name | Description | Inherited |
|------|-------------|-----------|
${validProperties.map((prop) => `| **${prop.name || 'Unknown'}** | ${prop.description || 'No description available'} | ${prop.inherited ? `Yes (from ${prop.inheritedFrom || 'Unknown'})` : 'No'} |`).join('\n')}
...
`;
            ...
            try {
                if (detailed.methods && Array.isArray(detailed.methods) && detailed.methods.length > 0) {
                    const validMethods = detailed.methods.filter((method) => method && typeof method === 'object');
                    if (validMethods.length > 0) {
                        output += `## Methods

| Name | Description | Inherited |
|------|-------------|-----------|
${validMethods.map((method) => `| **${method.name || 'Unknown'}** | ${method.description || 'No description available'} | ${method.inherited ? `Yes (from ${method.inheritedFrom || 'Unknown'})` : 'No'} |`).join('\n')}
...
`;
            ...
            try {
                if (detailed.examples && Array.isArray(detailed.examples) && detailed.examples.length > 0) {
                    const validExamples = detailed.examples.filter((example) => example);
                    if (validExamples.length > 0) {
                        output += `## Examples

${validExamples.map((example, index) => `### Example ${index + 1}
\`\`\`csharp
